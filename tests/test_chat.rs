use line_messaging_api::action::{LineActionUri, LineMessageActionCamera};
use line_messaging_api::message::{LineMessageImage, LineMessageImageMap, LineMessageImageMapActionUri, LineMessageImageMapArea, LineMessageImageMapBaseSize, LineMessageTemplate, LineMessageTemplateButton, LineMessageVideo, LineMessagesBuilder, LineMessageText, LineMessageTemplateConfirm, LineMessageTemplateCarousel, LineMessageTemplateCarouselColumn, LineMessageTemplateCarouselImage, LineMessageTemplateCarouselImageColumn, LineMessageFlex};
use line_messaging_api::{LineClient, LineError};
...
async fn test_リッチメニュー正常ケーステスト() {
    ...
    println!("{:?}",client.rich_menu_user_get_user_rich_menu_id(user_id).await);

    if let Ok(res) = client.rich_menu_delete_default_menu().await {} else {
        assert!(false);
    };
...
async fn test_配信正常ケーステスト() {
    ...
    let mut template = LineMessageTemplate::new(
        "alt",
        template_button,
    );
    ...
    let mut col1 = LineMessageTemplateCarouselImageColumn::new(test_image,LineActionUri::new("col1_url", test_url));
    let mut col2 = LineMessageTemplateCarouselImageColumn::new(test_image,LineActionUri::new("col2_url", test_url));
 ...
 // Flex
    let mut builder = LineMessagesBuilder::new();
    let mut template_confirm = LineMessageFlex::new("aa",json!( {
    "type": "bubble",
    "body": {
      "type": "box",
      "layout": "vertical",
    ...
    //
    //
    println!("{}", json!(builder.to_multi_cast_request(vec![user_id.to_string()])).to_string());
    // // let a = client.message_push(&builder.to_push_request(user_id)).await;
    let a = client.message_send_push(
        &builder.to_push_request(user_id),
    ).await;
    println!("{:?}", a);
}
...
async fn test_配信メッセージ集計() {
    ...
    println!("{:?}", res);
    let res = client.message_get_number_of_reply_messages("20231201").await;
    println!("{:?}", res);
    let res = client.message_get_number_of_push_messages("20231201").await;
    println!("{:?}", res);
    let res = client.message_get_number_of_multicast_messages("20231201").await;
    println!("{:?}", res);
    let res = client.message_get_number_of_broadcast_messages("20231201").await;
    println!("{:?}", res);
